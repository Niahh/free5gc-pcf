...
// newNrfTestConsumer builds a Consumer over a private PCF context pointed at the
// test NRF and intercepts the openapi cleartext HTTP/2 client with gock.
func newNrfTestConsumer(t *testing.T) *Consumer {
	t.Helper()

	consumer := newTestConsumer(t, &pcf_context.PCFContext{
		NfId:   testNfId,
		NrfUri: testNrfUri,
	})
...
func TestSendSearchNFInstances(t *testing.T) {
	...
	gock.New(testNrfUri).
		...
		MatchParam("requester-nf-type", "PCF").
		Reply(http.StatusOK).
	...
	consumer := newTestConsumer(t, &pcf_context.PCFContext{NrfUri: testNrfUri})

	result, err := consumer.SendSearchNFInstances(testNrfUri,
		models.Nrf_NFMgmt_NFType_UDR, models.Nrf_NFMgmt_NFType_PCF, NFDisc.SearchNFInstancesRequest{})
	if err != nil {
		t.Fatalf("SendSearchNFInstances: %v", err)
	}
	if !gock.IsDone() {
		t.Fatal("the discovery GET was not sent as expected")
	}
	if result.ValidityPeriod != 60 {
		t.Errorf("ValidityPeriod = %d, want 60", result.ValidityPeriod)
	}
	if len(result.NfInstances) != 0 {
		t.Errorf("NfInstances = %+v, want none", result.NfInstances)
	}
}
...
func TestSendUpdateNFInstanceWithoutNrfUri(t *testing.T) {
	consumer := newTestConsumer(t, &pcf_context.PCFContext{NfId: testNfId})
...
func TestRegisterNFInstance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The PCF keeps the instance ID it chose, whatever the NRF echoes back.
